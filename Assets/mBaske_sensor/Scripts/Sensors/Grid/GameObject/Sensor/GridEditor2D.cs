        ...
        private void DrawHandles()
        {
            ...
            using (new Handles.DrawingScope(matrix))
            {
                EditorGUI.BeginChangeCheck();
                {
                    ...
                    {
                        Handles.DrawLine(new Vector3(x, yMin, z), new Vector3(x, yMax, z));
                    }
                    for (float y = yMin + cellSize, max = yMax - cellSize * 0.5f; y < max; y += cellSize)
                    {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    b.size = new Vector3(
                        ...
                        Mathf.Max(b.size.y, cellSize),
                        Mathf.Max(b.size.z, 1));
